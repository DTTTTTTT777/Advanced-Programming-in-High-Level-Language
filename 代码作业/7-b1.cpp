...
{
	if ((prompt == NULL) || (strlen(prompt) == 0)) //思考一下，||的左右两个条件能否互换 不能 前者是执行后者的条件
		cout << endl << "按回车键继续";
	else
...
struct tj_time* tj_time_convert(int input_time)
{
	...
	const int DAYsecond = 60 * 60 * 24, HOURsecond = 60 * 60, MINsecond = 60;
	input_time += HOURsecond * 8;
	result.tj_year = 1970;
	while (input_time >= 365 * DAYsecond + is_leap(result.tj_year) * DAYsecond)
	{
		input_time -= 365 * DAYsecond + is_leap(result.tj_year) * DAYsecond;
		++result.tj_year;
	}
	const int days[13] = { 0,31,28,31,30,31,30,31,31,30,31,30,31 };
	result.tj_month = 1;
	while (input_time >= days[result.tj_month] * DAYsecond + int(result.tj_month == 2 && is_leap(result.tj_year)) * DAYsecond)
	{
		input_time -= days[result.tj_month] * DAYsecond + int(result.tj_month == 2 && is_leap(result.tj_year)) * DAYsecond;
		++result.tj_month;
	}
	result.tj_day = 1;
	while (input_time >= DAYsecond)
	{
		input_time -= DAYsecond;
		++result.tj_day;
	}
	result.tj_hour = 0;
	while (input_time >= HOURsecond)
	{
		input_time -= HOURsecond;
		++result.tj_hour;
	}
	result.tj_minute = 0;
	while (input_time >= MINsecond)
	{
		input_time -= MINsecond;
		++result.tj_minute;
	}
